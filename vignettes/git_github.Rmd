...
`C:\Users\<owner>\AppData\Local\Git\bin\git.exe` (in Windows notation; in R
these are `C:/Program Files/Git/bin/git.exe` or
`C:/Users/<owner>/AppData/Local/Git/bin/git.exe`, see the [Notes on paths]):
run `where git.exe` in the [shell](https://happygitwithr.com/shell) to get its
location.
...
typing `Git Bash` + `Enter` in Windows' `Start` menu.
...
a file, delete the file from the local repository (i.e., your PC), then `commit`
the change (i.e., deletion of a file), using the `commit message` to describe
why the file was deleted. Then `pull` to make sure the local repository is up to
...
- To download code without being able to push your changes back to a GitHub
  ...
  `C:\Program Files\R\R-4.5.2\library` or
  `C:\Users\<owner>\AppData\Local\R\win-library\4.5`. Then open the `.Rproj`
...
- GitHub help pages on 
  ...
  [setting up GitHub](https://docs.github.com/en/get-started/onboarding/getting-started-with-your-github-account), on [general](https://docs.github.com/en) help, and on
