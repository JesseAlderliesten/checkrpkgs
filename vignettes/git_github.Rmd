...
Create a [GitHub](https://github.com/) account, download a GitHub client such as
...
Open the `Git Bash` shell (which was installed when installing
...
See [Happy Git with R](https://happygitwithr.com/hello-git.html) for more details.
```{Git Bash}
...
- The Chapter about Personal Access Tokens in 
  [Happy Git and GitHub for the useR](https://happygitwithr.com/https-pat.html);
...
- in RStudio: check the checkbox in front of the relevant filename, use the
...
- in the `shell`: to compare the content of two files, see the instructions in
  section 'Comparing files' below. Next, use
...
'Using personal access tokens' above). After you have entered the PAT once,
...
R-scripts can be compared using the `Bash` shell: open the `Bash` shell and copy
the file path (i.e., the directories and file names, *including* the extensions)
...
## Downloading, forking, or cloning a GitHub repository
...
- To be able to push your changes back to a GitHub repository to which you do
  ...
  GitHub repository to your local computer by using the green button `Code` in
...
- The section [Fork and clone](https://happygitwithr.com/fork-and-clone) from
  `Happy Git with R`;
- The GitHub documentation about 
...
For an overview of some `shell` commands see the documentation of
[Git SCM](https://git-scm.com/docs) or in the `BASH shell` type `git config` or
...
- GitHub: [general](https://docs.github.com/en) help page;
  help page on 
  [setting up](https://docs.github.com/en/get-started/git-basics/set-up-git)
  GitHub;
- [Happy Git and GitHub for the useR](https://happygitwithr.com/index.html), by
  Jennifer Bryan;
- [RStudio](https://posit.co/products/open-source/rstudio/?sid=1) help page
  about
