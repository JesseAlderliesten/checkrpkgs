...
- To download code without being able to push your changes back to a GitHub
  ...
  choosing `Download ZIP`, unzip the downloaded files (right-click on them and
  choose `extract all`), and open the `.Rproj` file with the same name as the
  repository.

### Installing a package from GitHub
The following code can be used to install packages from
[GitHub](https://github.com/) (see the section
'R packages > Installing packages > Github' in the vignette
*Instructions about R packages* for details:
`vignette("r_pkgs", package = "checkrpkgs")`):

```{r, eval = FALSE}
remotes::install_github(repo = repo_pkgs_new, dependencies = NA, upgrade = "ask",
