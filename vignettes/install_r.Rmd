...
## R
To install [R](https://www.r-project.org/), select a nearby CRAN
[mirror](https://cran.r-project.org/mirrors.html)
(i.e., websites hosted in various parts of the world with the same content as
the [main](https://cran.r-project.org/) CRAN page), download R, e.g., via
...
- Error instead of silently use only the first element in comparisons
...
- Error instead of warn if a condition has length larger
...
- Print warnings immediately as they occur (default: `options(warn = 0)`):  
  `options(warn = 1)`.
- Make warnings an error (default: `options(warn = 0)`):  
  `options(warn = 2)`.
- Enter the environment browser (see `help(browser)`) upon error (default:
  `options(error = NULL)`):  
...
but software used to build R packages from source: packages from
...
- CRAN [mirrors](https://cran.r-project.org/mirrors.html) with information about
  their [status](https://cran.r-project.org/mirmon_report.html) also available
