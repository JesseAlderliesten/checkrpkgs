...
example, `<pkgname>` is used as a place holder to refer to a package name and
...
`methods`. Similarly, `<funcname>` is used as a place holder to refer to a
...
Note that R is *not* required to read R-scripts: R-scripts are plain-text files
that can be read by applications such as Microsoft NotePad.

### Configuring R
R can be configured by changing [general options](../help/options),
[environment variables](../help/environment%20variables), and options for
[startup](../help/Startup), [installing packages](../help/install.packages), and
[library paths](../help/.libPaths).

Configuring R can be used to made R a bit stricter, described in the help files
for [options](../help/options) and
[environment variables](../help/environment%20variables):

- Warn in case of [partial matching](../help/pmatch) such as `list(mean = 3)$me`:
  `options(warnPartialMatchArgs = TRUE, warnPartialMatchAttr = TRUE, warnPartialMatchDollar = TRUE)`. The default is `FALSE` for each of these.
- Error instead of warn when calling [a:b](../help/colon) when numeric `a` or `b`
  is longer than one, such as `3:c(5, 7)` (introduced in R 4.3.0):
  `Sys.setenv("_R_CHECK_LENGTH_COLON_" = "TRUE")`.
- Error instead of silently using only the first element in
  [logical operations](../help/Logic) such as `c(TRUE, TRUE) && TRUE)`
...
- Error instead of warn if a [condition](../help/Control) has length larger than
  one, such as `if(3 < c(5, 7)) {print("Not OK")}` (introduced in R 3.y.z, no
  longer used since R 4.2.0 because there conditions with length larger than one
  always give an error): `Sys.setenv("_R_CHECK_LENGTH_1_CONDITION_" = "TRUE")`.
- Print warnings [warnings](../help/warning) immediately as they occur:
  `options(warn = 1)`. The default is `options(warn = 0)` to warn after the
  top-level function returns.
- Make [warnings](../help/warning) an [error](../help/stop): `options(warn = 2)`.
  This should only be used for debugging because it may trigger bugs and
  resource leaks (per its help-page). The default is `options(warn = 0)` to warn
  after the top-level function returns.
...
- Package [strict](https://github.com/hadley/strict/) warns about various unsafe
  ...
  argument of length one and type-unsafe [sapply()](../help/sapply); additional
  ideas are in the [issues](https://github.com/hadley/strict/issues?q=is%3Aissue).
- Package [conflicted](https://CRAN.R-project.org/package=conflicted) avoids
  silent [conflict](../help/conflicts) resolution (i.e., choosing the latest
  attached package out of multiple packages to use a function from), and provides
...
- [.Machine](../help/.Machine) and [Sys.info()](../help/Sys.info) provide
  information about the machine and platform R is running on.
  [getRversion()](../help/R.Version) provides the version of the running R.
- [.Platform](../help/.Platform) and [R.Version()](../help/R.Version) provide
  information about the platform R was built on.
- [Sys.getlocale()](../help/Sys.getlocale) provides details about the locale.
- [sessionInfo()](../help/sessionInfo) extracts parts of the information
  provided by the functions mentioned above about the operating system and R. It
  also lists attached and loaded packages. Its printing method can be used to
  print additional information about the used locale, time zone, and random
  number generation: `print(sessionInfo(), locale = TRUE, RNG = TRUE)`.
- [capabilities()](../help/capabilities) and
  [extSoftVersion()](../help/extSoftVersion) provide details about external
  software that can be used with R.
- [environment variables](../help/environment%20variables) lists some of the
  environment variables which affect an R session.
...
[sessionInfo()](../help/sessionInfo) and the `Note` in
[win.version()](../help/win.version)).
...
light `Editor theme` is `Xcode`. Nice dark `Editor theme`s are
`Tomorrow Night Bright`, `Idle Fingers`, and `Pastel On Dark`, with
...
(see the section 'Installing packages' in the vignette
*Instructions about R packages* for instructions:
`vignette("r_pkgs", package = "checkrpkgs")`). Rtools is *not* needed to install
current versions of packages from
[CRAN](https://cran.r-project.org/web/packages/index.html) or
...
[getRversion()](../help/R.Version)) from [CRAN](https://cran.r-project.org/)
