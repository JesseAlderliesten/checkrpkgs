...
### Configuring R
R can be configured to be a bit stricter, described in the help files for
general options (`help("options")`) and environment variables
...
- Error instead of warn if a condition (`help("Control", package = "base")`) has
  length larger than one, such as `if(3 < c(5, 7)) {print("Not OK")}`
  (introduced in R 3.y.z, no longer used since R 4.2.0 because there conditions
  with length larger than one always give an error):
  `Sys.setenv("_R_CHECK_LENGTH_1_CONDITION_" = "TRUE")`.
- Print `warnings()` immediately as they occur (`options(warn = 1)`) or make
  `warnings` an error (`help("stop")`): `options(warn = 2)`. The latter should
...
Finally, when using options for start-up (described in `help("Startup")`) and
installing packages (`help("install.packages")` and `help(".libPaths")`) to
configure R, it should be kept in mind that these options might lead to code
that behaves different on PCs where these options are not set, for example when
they do not automatically load a package or add a path to the search path.
...
- `sessionInfo()` extracts parts of the information provided by the functions
  ...
  information about the used locale, time zone, and random number generation:
...
Operating systems might identify themselves and their versions in surprising
ways, and specifically Windows versions might report older versions than the
...
`Idle Fingers`, and `Pastel On Dark`. Nice `Editor fonts` are `Consolas`,
...
- My package [develcoder](https://jessealderliesten.github.io/develcoder/)
