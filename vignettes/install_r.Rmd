...
- Error instead of warn if a [condition](../help/Control) has length larger than
  one, such as `if(3 < c(5, 7)) {"Now run some code"}` (introduced in R 3.y.z, no
...
- Print warnings [warnings](../help/warning) immediately as they occur (default:
  `options(warn = 0)`): `options(warn = 1)`.
- Make [warnings](../help/warning) an [error](../help/stop) (default:
...
- [sessionInfo()](../help/sessionInfo) extracts parts of the information
  ...
  print additional information: `print(sessionInfo(), locale = TRUE, RNG = TRUE)`.
