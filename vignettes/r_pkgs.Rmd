...
if(length(pkgs_install) > 0L) {
  install.packages(pkgs = pkgs_install, lib = NULL, dependencies = NA,
...
install.packages(pkgs = "https://cran.r-project.org/src/contrib/Archive/deSolve/deSolve_1.40.tar.gz",
                 lib = NULL, repos = NULL, dependencies = NA, type = "source",
...
BiocManager::install(pkgs = pkgs_new, lib = NULL, dependencies = NA,
                     type = getOption("pkgType"), verbose = getOption("verbose"),
...
```{r, eval = FALSE}
utils::update.packages(lib.loc = NULL, ask = TRUE, dependencies = NA,
                       verbose = getOption("verbose"), quiet = FALSE,
...
installing or updating. If a package is non-functional, use
`get_details_pkgs()` to check what dependencies it has and then use
`find_nonfunc_pkgs()` to see if these are all installed.
...
For operators such as `%in%`, use backticks around the name. To differentiate
functions with the same name defined in different packages, specify the package
followed by two colons: `<pkgname>::<funcname>`. For non-exported (i.e.,
internal) functions, specify the package followed by three colons:
`<pkgname>:::<funcname>`. Note that non-exported functions should *not* be used:
they are non-exported because they might change.
...
- The help-page `help(checkrpkgs::find_nonfunc_pkgs)`.
