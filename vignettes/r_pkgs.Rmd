...
provide thematic collections of packages:

- [Bioconductor](https://bioconductor.org/) with 
  [thematic collections](https://bioconductor.org/packages/release/BiocViews.html#___Software);
- [CRAN](https://cran.r-project.org/web/packages/index.html) or any of its
  ...
  various parts of the world with the same content as the main CRAN website;
  with thematic collections organised in
...
`remotes::install_version(<pkgname>, version = "1.40", upgrade = "ask")`.
...
`install.packages(pkgs = "https://cran.r-project.org/src/contrib/Archive/deSolve/deSolve_1.40.tar.gz", repos = NULL, type = "source")`.
...
`getAnywhere("<funcname>")`: that function
...
use `getAnywhere("<function.class>")`, where `<function.class>` is an item from
the overview returned by `methods("<funcname>")`, to obtain the source code of a
...
function has different [S4-methods][methods::Introduction] for different object
...
search in the `src` folder of the downloaded code.


## Troubleshooting
Note that package names are case-sensitive, just like everything else in R.
